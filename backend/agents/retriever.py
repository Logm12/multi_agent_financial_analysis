...
def retrieve_node(state: AgentState, config: RunnableConfig = None) -> dict:
    ...
    try:
        ...
        rewriter_prompt = PromptTemplate.from_template(
            "You are a search expert for financial statement database. Translate the following user query into a concise search query (space-separated keywords), removing conversational fluff. Focus on the target company/ticker symbol (e.g. VNM, FPT) and specific financial metrics.\n\nCRITICAL 1: The search keywords MUST be in the same language as the query (e.g., if query is in Vietnamese, output keywords in Vietnamese like 'tài sản dài hạn VNM').\nCRITICAL 2: If the query asks for a calculated financial ratio (like ROE, ROA, liquidity ratios, etc.), do NOT just search for the ratio name. Instead, break it down and output the search keywords of its component variables (e.g., for ROE, include keywords like 'Lợi nhuận sau thuế', 'Vốn chủ sở hữu' along with the ticker; for ROA, 'Lợi nhuận sau thuế', 'Tổng tài sản').\n\nOutput ONLY the space-separated keywords on a single line. Do NOT include any explanation, formatting, or labels.\n\nQuery: {query}\nKeywords:"
        )
